$(document).ready(function(){
    ...
    $(document).on('click', '[data-action=delete_link]', function(event){
        ...
        $.post(this.href, function(data){
            ...
            if (data.ret) {
                $(a_cont).empty().append('<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button><h4>Ошибка!</h4>'+data.error+'</div>');
            } else {
                ...
                $(a_cont).empty().append('<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button><h4>Готово!</h4>'+data.message+'</div>');
            }
        }, 'json');
        return false;
    });
